      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6">
        ...
        <div className="md:hidden mb-8">
          ...
          <div className="flex flex-col items-center text-center mb-6 pb-6 border-b border-primary-700/50">
            <img src={logoDark} alt="مسجدي" className="h-10 w-auto object-contain mb-3" />
          ...
          <div className="grid grid-cols-2 gap-6">
            ...
            <div>
              ...
              <ul className="space-y-2.5 text-primary-300 text-sm">
                <li><Link to="/" className="hover:text-white transition-colors flex items-center gap-1.5"><i className="fas fa-chevron-left text-xs text-primary-500"></i>الرئيسية</Link></li>
                <li><Link to="/attendance" className="hover:text-white transition-colors flex items-center gap-1.5"><i className="fas fa-chevron-left text-xs text-primary-500"></i>الحضور</Link></li>
                <li><Link to="/contact" className="hover:text-white transition-colors flex items-center gap-1.5"><i className="fas fa-chevron-left text-xs text-primary-500"></i>تواصل معنا</Link></li>
                <li><Link to="/login" className="hover:text-white transition-colors flex items-center gap-1.5"><i className="fas fa-chevron-left text-xs text-primary-500"></i>تسجيل الدخول</Link></li>
              </ul>
            </div>
              ...
              <ul className="space-y-2.5 text-primary-300 text-sm">
                ...
                {contact.email && (
                  <li className="flex items-center gap-2">
                    ...
                    <a href={`mailto:${contact.email}`} className="hover:text-white text-xs truncate">{contact.email}</a>
                  </li>
                )}
              </ul>
              {(contact.facebook || contact.instagram || contact.twitter) && (
                <div className="flex gap-2 mt-3">
                  {contact.facebook  && <a href={contact.facebook}  target="_blank" rel="noreferrer" className="w-7 h-7 bg-white/10 rounded-full flex items-center justify-center hover:bg-white/20 transition-colors"><i className="fab fa-facebook-f text-xs"></i></a>}
                  {contact.instagram && <a href={contact.instagram} target="_blank" rel="noreferrer" className="w-7 h-7 bg-white/10 rounded-full flex items-center justify-center hover:bg-white/20 transition-colors"><i className="fab fa-instagram text-xs"></i></a>}
                  {contact.twitter   && <a href={contact.twitter}   target="_blank" rel="noreferrer" className="w-7 h-7 bg-white/10 rounded-full flex items-center justify-center hover:bg-white/20 transition-colors"><i className="fab fa-twitter text-xs"></i></a>}
                </div>
              )}
            </div>
