...
export default function TeacherAttendance() {
  ...
  const statusOptions = [['present','حاضر','bg-green-100 text-green-700 dark:bg-green-900/30 dark:text-green-400'],['absent','غائب','bg-red-100 text-red-700 dark:bg-red-900/30 dark:text-red-400'],['excused','معذور','bg-yellow-100 text-yellow-700 dark:bg-yellow-900/30 dark:text-yellow-400']];
      ...
      <div className="bg-white dark:bg-[#1a2d1e] rounded-2xl shadow-md dark:shadow-black/30 p-5 mb-6 dark:border dark:border-primary-900/40">
        ...
        {selectedClass && (
          <div className="flex gap-3 flex-wrap">
            <button onClick={() => { const a = {}; students.forEach(s => a[s._id]='present'); setAttendance(a); }} className="text-xs px-3 py-1.5 bg-green-100 dark:bg-green-900/30 text-green-700 dark:text-green-400 rounded-lg font-semibold hover:bg-green-200 dark:hover:bg-green-900/50">تحديد الكل حاضر</button>
            <button onClick={() => { const a = {}; students.forEach(s => a[s._id]='absent'); setAttendance(a); }} className="text-xs px-3 py-1.5 bg-red-100 dark:bg-red-900/30 text-red-700 dark:text-red-400 rounded-lg font-semibold hover:bg-red-200 dark:hover:bg-red-900/50">تحديد الكل غائب</button>
          </div>
        )}
      </div>

      {!selectedClass ? (
        <div className="text-center py-16 text-gray-400 dark:text-gray-500 bg-white dark:bg-[#1a2d1e] rounded-2xl shadow-md dark:border dark:border-primary-900/40"><i className="fas fa-chalkboard-teacher text-5xl mb-3"></i><p>اختر فصلاً للبدء</p></div>
      ) : filtered.length === 0 ? (
        <div className="text-center py-16 text-gray-400 dark:text-gray-500 bg-white dark:bg-[#1a2d1e] rounded-2xl shadow-md dark:border dark:border-primary-900/40"><i className="fas fa-user-graduate text-5xl mb-3"></i><p>لا يوجد طلاب في هذا الفصل</p></div>
      ) : (
        <>
          <div className="bg-white dark:bg-[#1a2d1e] rounded-2xl shadow-md dark:shadow-black/30 overflow-hidden mb-4 dark:border dark:border-primary-900/40">
            <div className="p-4 bg-primary-50 dark:bg-primary-900/30 border-b dark:border-primary-900/40 flex items-center justify-between">
              ...
              <div className="flex gap-3 text-xs">
                <span className="bg-green-100 dark:bg-green-900/30 text-green-700 dark:text-green-400 px-2 py-1 rounded-full">حاضر: {Object.values(attendance).filter(v => v === 'present').length}</span>
                <span className="bg-red-100 dark:bg-red-900/30 text-red-700 dark:text-red-400 px-2 py-1 rounded-full">غائب: {Object.values(attendance).filter(v => v === 'absent').length}</span>
              </div>
            </div>
