        ...
        <div className="max-w-5xl w-full">
          ...
          <div className="flex flex-col md:flex-row items-center gap-10 md:gap-16">
            ...
            <div className="flex-shrink-0 flex justify-center w-full md:w-auto"
              ...
              <div className="relative">
                ...
                <div className="absolute -inset-8 rounded-[3.5rem] pointer-events-none"
                     style={{
                       background: 'radial-gradient(ellipse at center, rgba(34,197,94,0.18) 0%, rgba(21,128,61,0.08) 55%, transparent 75%)',
                       animation: 'amjPulse 3s ease-in-out infinite',
                     }} />

                {/* Layer 2 — rotating conic gradient ring */}
                <div className="absolute -inset-1 rounded-[2rem] pointer-events-none overflow-hidden"
                     style={{ padding: '2px', animation: 'amjSpin 6s linear infinite' }}>
                  <div style={{
                    width: '100%', height: '100%', borderRadius: '1.875rem',
                    background: 'conic-gradient(from 0deg, transparent 0%, rgba(34,197,94,0.7) 20%, rgba(250,204,21,0.5) 35%, transparent 50%, rgba(34,197,94,0.4) 70%, transparent 100%)',
                  }} />
                </div>

                {/* Layer 3 — static soft green border glow */}
                <div className="absolute -inset-0.5 rounded-[1.85rem] pointer-events-none"
                     style={{ background: 'rgba(34,197,94,0.25)', filter: 'blur(6px)' }} />

                {/* Logo card */}
                <div className="relative w-64 h-64 sm:w-80 sm:h-80 md:w-96 md:h-96 rounded-[1.75rem] overflow-hidden shadow-2xl shadow-primary-900/60"
                     style={{ border: '1.5px solid rgba(34,197,94,0.3)' }}>
                  <img src={amjLogo} alt="AMJ" className="w-full h-full object-cover" />

                  {/* Sweep shimmer */}
                  <div className="absolute inset-0 pointer-events-none"
                       style={{ animation: 'amjSweep 4s ease-in-out infinite',
                                background: 'linear-gradient(105deg, transparent 35%, rgba(255,255,255,0.07) 50%, transparent 65%)' }} />

                  {/* Bottom vignette */}
                  <div className="absolute inset-0 pointer-events-none"
                       style={{ background: 'linear-gradient(to top, rgba(10,40,18,0.35) 0%, transparent 50%)' }} />
                </div>

                {/* Bottom ground reflection */}
                <div className="absolute -bottom-2 left-1/2 -translate-x-1/2 w-3/4 h-4 rounded-full pointer-events-none"
                     style={{ background: 'rgba(34,197,94,0.2)', filter: 'blur(14px)', animation: 'amjPulse 3s ease-in-out infinite' }} />
              </div>
            </div>
            ...
            <div className="flex-1 w-full text-center md:text-right" dir="rtl">
              ...
              <p className="text-gray-400 font-medium mb-5 text-xs sm:text-sm" dir="ltr">
              ...
              {loading ? (
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-2.5">
                  ...
                  {contact.phone && (
                    <a href={`tel:${contact.phone}`}
                      ...
                      <span className="text-gray-700 dark:text-gray-200 font-medium text-sm truncate" dir="ltr">{contact.phone}</span>
                    </a>
                  )}
                  ...
                  {contact.workPlatform && (
                    <a href={contact.workPlatform} target="_blank" rel="noreferrer"
                ...
                <p className="text-gray-400 text-sm text-center md:text-right">تعذّر تحميل بيانات التواصل</p>
              )}
            </div>
          </div>
          ...
          <div className="mt-14 sm:mt-20 pt-6 border-t border-gray-100 dark:border-primary-900/40 text-center">
            <p className="text-gray-400 dark:text-gray-500 text-xs sm:text-sm flex items-center justify-center gap-2 flex-wrap">
              ...
              <span className="inline-flex items-center gap-1.5 font-bold text-primary-700 dark:text-primary-300">
                <img src={amjLogo} alt="AMJ" className="w-5 h-5 rounded-md object-cover inline-block" />
                أحمد الجاسم — AMJ
              </span>
            </p>
          </div>
